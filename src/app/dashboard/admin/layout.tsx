...
import { LayoutDashboard, Users, BookOpen, Calendar, Menu, X, ChevronRight, LogOut, ArrowLeft, Shield } from "lucide-react";
...
const sidebarLinks = [
  ...
  { name: "Categories", href: "/dashboard/admin/categories", icon: BookOpen },
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="max-w-md w-full p-12 bg-white rounded-[3rem] shadow-2xl border border-gray-100 text-center">
          ...
          <p className="text-gray-500 mb-10 leading-relaxed">Only administrators can access this area.</p>
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row">
      ...
      <div className="flex items-center justify-between bg-white px-4 sm:px-8 py-4 border-b border-gray-100 sticky top-0 z-[60] shadow-sm md:hidden">
        ...
        <div className="flex items-center space-x-3">
          <div className="w-10 h-10 bg-slate-100 text-slate-600 rounded-full flex items-center justify-center font-black text-sm border-2 border-white shadow-sm shrink-0">
            {user.name[0]}
          </div>
          <button
            onClick={logout}
         ...
         <div className="flex items-center space-x-6">
            <div className="flex items-center space-x-4">
               <div className="flex flex-col items-end">
                  <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest leading-none mb-1">Administrator</span>
        ...
        <div className="p-4 sm:p-6 lg:p-10 max-w-6xl mx-auto space-y-6 lg:space-y-10">
          <div className="flex flex-col sm:flex-row items-center justify-between bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm gap-6">
            <div className="flex items-center space-x-4 sm:space-x-6 w-full sm:w-auto">
              <div className="w-12 h-12 sm:w-14 sm:h-14 bg-gradient-to-br from-blue-700 to-blue-900 text-white rounded-2xl flex items-center justify-center font-black text-lg sm:text-xl shadow-lg shadow-blue-200 shrink-0">
                <Shield size={28} />
              </div>
              <div className="min-w-0">
                <h2 className="text-xl sm:text-2xl font-black text-gray-900 tracking-tight truncate">Admin Console</h2>
                <p className="text-[10px] sm:text-sm text-gray-400 font-bold tracking-widest uppercase truncate">System Control • {user.name}</p>
              </div>
            </div>
            <div className="flex items-center space-x-2 sm:space-x-4 text-[10px] font-black uppercase tracking-widest text-gray-400 w-full sm:w-auto overflow-x-auto no-scrollbar pb-1 sm:pb-0">
               <span className="bg-blue-50 text-blue-600 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full border border-blue-100 whitespace-nowrap">Super Admin</span>
               <span className="bg-emerald-50 text-emerald-600 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full border border-emerald-100 whitespace-nowrap">Secure Session</span>
            </div>
          </div>
